...
```{r combine-solution}
port_ret_both<- port_ret_vw %>%
  left_join(port_ret_ew,by="date")

port_ret_both<-port_ret_both %>%
  rename(ret_vw="ret.x",ret_ew="ret.y")
```
...
port_ret_both %>%
  ungroup() %>%
  drop_na() %>%
  mutate(wealth_vw=cumsum(ret_vw),
         wealth_ew=cumsum(ret_ew)) %>%
